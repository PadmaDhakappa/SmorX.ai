...
import { getSaasCategory, SAAS_CATEGORIES } from '../data/saas'
...
export default function SaaSDetail() {
  ...
  const category = getSaasCategory(slug)

  useEffect(() => { window.scrollTo(0, 0) }, [slug])

  if (!category) return <Navigate to="/saas" replace />

  const { label, icon: Icon, color, tagline, overview, agentLayer, useCase, impactStat, ctaLabel } = category

  const otherCategories = SAAS_CATEGORIES.filter(c => c.slug !== category.slug).slice(0, 3)

  return (
    <div style={{ background: '#060A12', minHeight: '100vh' }}>
      <SEO
        title={`${label} — AI-Layered SaaS Development`}
        description={category.gridDesc}
        canonical={`/saas/${category.slug}`}
        breadcrumbs={[
          { name: 'Home', path: '/' },
          { name: 'SaaS Solutions', path: '/saas' },
          { name: label, path: `/saas/${category.slug}` },
        ]}
      />
      <Navbar />
      <main id="main-content">
        ...
        <section className="relative pt-40 pb-16 overflow-hidden" style={{ background: '#060A12' }}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center gap-5">
            <Link to="/saas" className="text-xs font-medium text-white/55 hover:text-white/80 transition-colors duration-200">
        ...
        <section className="relative py-16 overflow-hidden" style={{ background: '#0B1020' }}>
          <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-[10px] font-semibold text-white/50 uppercase tracking-wider mb-5">Explore Other Categories</div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {otherCategories.map(c => (
                ...
                >
                  ...
                  <span className="text-sm text-white/70 leading-snug">{c.label}</span>
                </Link>
              ))}
            </div>
          </div>
